...
- **自选/选股联动**：支持“全部”列表和“自选”列表的橡皮筋多选，右键可批量加入自选、移除或移动分组；自选列表展示最新价/涨跌幅且切分组同步刷新。
...
8. **管理自选**：
   - 在“全部”列表中拖拽或 Shift/Ctrl 多选后右键，选择目标自选分组即可批量加入；若无分组，可在菜单中进入“管理/新建分组…”创建。
   - 在“自选”列表中同样支持拖拽多选，右键可“移除选中”或“移动到分组”，分组切换后列表自动刷新。
